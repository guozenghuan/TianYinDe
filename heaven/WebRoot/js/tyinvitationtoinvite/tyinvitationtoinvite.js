$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '联系电话', name: 'phone', width: 2},
			{ label: '赴邀人数', name: 'number', width: 1},
			...
			{ label: '操作',  name: 'id',width: 2, formatter: function(value, options, row){
				return '<a class="btn btn-primary" style="padding: 6px 6px;margin-left: 6px;margin-top: 3px;" href="tyinvitationtoinvite_add.html?id='+row.id+'">编辑信息</a>'+
...
var vm = new Vue({
	...
	methods: {
		...
		update: function (event) {
			...
			location.href = "tyinvitationtoinvite_add.html?id="+id;
		}
	}
});
...
function delThisById(id){
	...
	confirm('确定要删除选中的记录？', function(){
		$.ajax({
			type: "POST",
		    url: "../tyinvitationtoinvite/delete",
