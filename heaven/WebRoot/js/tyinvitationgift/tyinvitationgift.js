$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '赠送的礼物', name: 'giftName', width: 1},
			{ label: '礼物价格', name: 'price', width: 1},
			...
			{ label: '操作',  name: 'id',width: 1, formatter: function(value, options, row){
				return '<a class="btn btn-primary" style="padding: 6px 6px;margin-left: 6px;margin-top: 3px;" href="tyinvitationgift_add.html?id='+row.id+'">编辑信息</a>'+
...
var vm = new Vue({
	...
	methods: {
		...
		update: function (event) {
			...
			location.href = "tyinvitationgift_add.html?id="+id;
		}
	}
});
